<script lang="ts">
	...
	/**
	 * @var {Assignation[] | null} chatAssignations The chat assignments
	 * (used only to set the value of chat.getAssignations() when checking for nullity)
	 */
	let chatAssignations: Assignation[] | null;
	/**
	...
	function showUserSettings(event: CustomEvent<any>): void {
		...
		const assignation = event.detail.assignation;
		// If there is no assignment (which should not be possible), exit the process
		const assignations = chat?.getAssignations();
		...
		if (assignation == null || assignation.getUser() == selectedUser) {
			selectedUser = null;
			return;
		}
		const user = assignation.getUser();
		if (!user) {
			toasts.add(createToast(
		...
		userOptions = [];
		if (assignation.getRole()?.getId() == Role.TYPE.GUEST.getId()) {
			userOptions.push({
			...
			if (assignation.getRole()?.getId() == Role.TYPE.ADMIN.getId()) {
				userOptions.push({
					text:
						"Give the role " +
	...
	$: {
		...
		if (chat) {
			chatAssignations = chat.getAssignations();
		}
	}
</script>
...
{:else}
	<h2>{chat.getName()}</h2>
	{#if chatAssignations && chatAssignations.some((assignation) => assignation
					.getUser()
