...
export async function isUserInChat(user: User, chat: Chat): Promise<boolean> {
	const existingAssignations = await api.getDataByFilter(
		Assignation.ENDPOINT,
		{ chat: chat.getId(), user: user.getId() }
	);
	if (existingAssignations !== null && existingAssignations.length > 0) {
		return true;
	}
	return false;
}
...
export async function joinChat(user: User, chat: Chat): Promise<boolean> {
	...
	const data = await assignation.sendToAPI();
	if (data !== null) {
		return true;
	} else {
