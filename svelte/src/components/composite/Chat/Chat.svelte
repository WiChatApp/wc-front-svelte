<script lang="ts">
	...
	import type { Assignation } from "../../../common/classes/Assignation";
	...
	/**
	 * @var {Assignation[]} assignations The chat assignations
	 */
	let assignations: Assignation[] | null;
	/**
	 ...
	 * @param {Assignation} assignation The member's assignation
	 *
	 * @returns {boolean} Returns false if the event was stopped
	 */
	function dispatchMemberSettingsClick(
		...
		return dispatch("memberSettingsClick", { event, assignation });
	}
	...
	$: {
		if (chat) {
			...
			assignations = chat.getAssignations();
		}
	}
</script>
	...
	<div class="chat-messages">
		...
		<div class="chat-members-list" id="msgList">
			{#if assignations}
				...
				{:else}
					{#each assignations as assignation}
						<Member
							{assignation}
							on:settingsClick={(event) => {
								dispatchMemberSettingsClick(event, assignation);
							}}
						/>
					{/each}
				{/if}
